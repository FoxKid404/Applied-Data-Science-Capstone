...
# Add outcome column for better visualization
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        fig = px.pie(spacex_df, names='outcome', title='Total Launches for All Sites')
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        fig = px.pie(filtered_df, names='outcome', title=f'Launches for site {entered_site}')
    return fig
...
def update_scatter_chart(entered_site, payload_range):
    ...
    fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='outcome', 
                     color='Booster Version Category', title='Payload vs. Outcome')
    return fig
